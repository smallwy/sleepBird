...
public  class Gof23_sington {
    ...
    private static Gof23_sington sigton = new Gof23_sington();

  /*  public  Gof23之单例模式(){

    }*/

    private static Gof23_sington getSigton() {
        return sigton;
    }
      ...
      /*------------------------懒汉式式创建实例对象-----------*/
    //缺点 存在线程安全问题
    private static Gof23_sington lanhanshi = null;
    public static synchronized Gof23_sington getLanhanshi() {
        if (lanhanshi == null) {
            lanhanshi = new Gof23_sington();
        }
        return lanhanshi;
    }
    ...
    private static Gof23_sington doublecheck = null;

    public Gof23_sington() {
        //这里的null没有赋值  实际上是赋值了
        this.conn = null;
    }

    public static Gof23_sington getSington() {
        if (doublecheck == null) {
            synchronized (Gof23_sington.class) {
                if (doublecheck == null) {
                    doublecheck = new Gof23_sington();
                }
  ...
  private static Gof23_sington getHolderSington(){
  ...
  public void test_懒汉式单列() {
        System.out.println(Gof23_sington.getLanhanshi());
    }
  ...
  public void test_checkDouble单列() {
        System.out.println(Gof23_sington.getSington());
    }

  /**通过内部类创建对象来实现懒加载  类在初始化的时候并不会实例化对象  只要我们调用的时候才开始创建 */
  @Test
  public void test_holder单列() {
        System.out.println(Gof23_sington.getHolderSington());
    }
