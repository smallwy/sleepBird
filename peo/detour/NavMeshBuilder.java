    ...
    private static int createBVTree(NavMeshDataCreateParams params, BVNode[] nodes) {
        ...
        for (int i = 0; i < params.polyCount; i++) {
            ...
            if (params.detailMeshes != null) {
                ...
                it.bmin[0] = DetourCommon.clamp((int) ((bmin[0] - params.bmin[0]) * quantFactor), 0, 0xffff);
                it.bmin[1] = DetourCommon.clamp((int) ((bmin[1] - params.bmin[1]) * quantFactor), 0, 0xffff);
                it.bmin[2] = DetourCommon.clamp((int) ((bmin[2] - params.bmin[2]) * quantFactor), 0, 0xffff);

                it.bmax[0] = DetourCommon.clamp((int) ((bmax[0] - params.bmin[0]) * quantFactor), 0, 0xffff);
                it.bmax[1] = DetourCommon.clamp((int) ((bmax[1] - params.bmin[1]) * quantFactor), 0, 0xffff);
                it.bmax[2] = DetourCommon.clamp((int) ((bmax[2] - params.bmin[2]) * quantFactor), 0, 0xffff);
            } else {
