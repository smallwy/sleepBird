...
public class CrowdAgent {
    ...
    boolean overOffmeshConnection(float radius) {
        ...
        if (offMeshConnection) {
            float distSq = DetourCommon.vDist2DSqr(npos, corners.get(corners.size() - 1).getPos());
    ...
    float getDistanceToGoal(float range) {
        ...
        boolean endOfPath = ((corners.get(corners.size() - 1).getFlags() & NavMeshQuery.DT_STRAIGHTPATH_END) != 0)
                ...
                : false;
        if (endOfPath)
            return Math.min(DetourCommon.vDist2D(npos, corners.get(corners.size() - 1).getPos()), range);
    ...
    public float[] calcSmoothSteerDirection() {
        ...
        if (!corners.isEmpty()) {
            ...
            float[] dir0 = DetourCommon.vSub(p0, npos);
            float[] dir1 = DetourCommon.vSub(p1, npos);
            dir0[1] = 0;
            dir1[1] = 0;
            ...
            if (len1 > 0.001f)
                dir1 = DetourCommon.vScale(dir1, 1.0f / len1);
    ...
    public float[] calcStraightSteerDirection() {
        float[] dir = new float[3];
        if (!corners.isEmpty()) {
            dir = DetourCommon.vSub(corners.get(0).getPos(), npos);
            dir[1] = 0;
            DetourCommon.vNormalize(dir);
        }
        return dir;
