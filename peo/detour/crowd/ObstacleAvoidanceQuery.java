...
public class ObstacleAvoidanceQuery {
    ...
    private void prepare(float[] pos, float[] dvel) {
        ...
        for (int i = 0; i < m_nsegments; ++i) {
            ...
            float r = 0.01f;
            Tupple2<Float, Float> dt = DetourCommon.distancePtSegSqr2D(pos, seg.p, seg.q);
            seg.touch = dt.first < DetourCommon.sqr(r);
        }
    }

    SweepCircleCircleResult sweepCircleCircle(float[] c0, float r0, float[] v, float[] c1, float r1) {
        final float EPS = 0.0001f;
        float[] s = DetourCommon.vSub(c1, c0);
        float r = r0 + r1;
        float c = DetourCommon.vDot2D(s, s) - r * r;
        float a = DetourCommon.vDot2D(v, v);
        if (a < EPS)
            return new SweepCircleCircleResult(false, 0f, 0f); // not moving

        // Overlap, calc time to exit.
        float b = DetourCommon.vDot2D(v, s);
        float d = b * b - a * c;
        if (d < 0.0f)
    ...
    Tupple2<Boolean, Float> isectRaySeg(float[] ap, float[] u, float[] bp, float[] bq) {
        float[] v = DetourCommon.vSub(bq, bp);
        float[] w = DetourCommon.vSub(ap, bp);
        float d = DetourCommon.vPerp2D(u, v);
        if (Math.abs(d) < 1e-6f)
            return new Tupple2<>(false, 0f);
        d = 1.0f / d;
        float t = DetourCommon.vPerp2D(v, w) * d;
        if (t < 0 || t > 1)
            return new Tupple2<>(false, 0f);
        float s = DetourCommon.vPerp2D(u, w) * d;
        if (s < 0 || s > 1)
            return new Tupple2<>(false, 0f);
    ...
    private float processSample(float[] vcand, float cs, float[] pos, float rad, float[] vel, float[] dvel,
        ...
        for (int i = 0; i < m_ncircles; ++i) {
            ...
            // RVO
            float[] vab = DetourCommon.vScale(vcand, 2);
            vab = DetourCommon.vSub(vab, vel);
            vab = DetourCommon.vSub(vab, cir.vel);

            // Side
            side += DetourCommon.clamp(Math.min(DetourCommon.vDot2D(cir.dp, vab) * 0.5f + 0.5f, DetourCommon.vDot2D(cir.np, vab) * 2), 0.0f, 1.0f);
            nside++;
        ...
        for (int i = 0; i < m_nsegments; ++i) {
            ...
            if (seg.touch) {
                ...
                float[] sdir = DetourCommon.vSub(seg.q, seg.p);
                float[] snorm = new float[3];
                snorm[0] = -sdir[2];
                snorm[2] = sdir[0];
                // If the velocity is pointing towards the segment, no collision.
                if (DetourCommon.vDot2D(snorm, vcand) < 0.0f)
                    continue;
    ...
    public Tupple2<Integer, float[]> sampleVelocityGrid(float[] pos, float rad, float vmax, float[] vel, float[] dvel,
        ...
        for (int y = 0; y < m_params.gridSize; ++y) {
            for (int x = 0; x < m_params.gridSize; ++x) {
                ...
                DetourCommon.vSet(vcand, cvx + x * cs - half, 0f, cvz + y * cs - half);
    ...
    public Tupple2<Integer, float[]> sampleVelocityAdaptive(float[] pos, float rad, float vmax, float[] vel,
        ...
        DetourCommon.vSet(res, dvel[0] * m_params.velBias, 0, dvel[2] * m_params.velBias);
        int ns = 0;
        for (int k = 0; k < depth; ++k) {
            ...
            for (int i = 0; i < npat; ++i) {
                float[] vcand = new float[3];
                DetourCommon.vSet(vcand, res[0] + pat[i * 2 + 0] * cr, 0f, res[2] + pat[i * 2 + 1] * cr);
                if (DetourCommon.sqr(vcand[0]) + DetourCommon.sqr(vcand[2]) > DetourCommon.sqr(vmax + 0.001f))
                    continue;
