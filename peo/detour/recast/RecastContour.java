...
public class RecastContour {
    ...
    private static class ContourRegion {
        public org.recast4j.recast.Contour outline;
    ...
    private static class ContourHole {
        ...
        public int minx;
        public int minz;
        public org.recast4j.recast.Contour contour;
    }
    ...
    private static int getCornerHeight(int x, int y, int i, int dir, CompactHeightfield chf, boolean isBorderVertex) {
        ...
        if (org.recast4j.recast.RecastCommon.GetCon(s, dir) != RC_NOT_CONNECTED) {
            ...
            if (org.recast4j.recast.RecastCommon.GetCon(as, dirp) != RC_NOT_CONNECTED) {
                int ax2 = ax + org.recast4j.recast.RecastCommon.GetDirOffsetX(dirp);
                int ay2 = ay + org.recast4j.recast.RecastCommon.GetDirOffsetY(dirp);
                int ai2 = chf.cells[ax2 + ay2 * chf.width].index + org.recast4j.recast.RecastCommon.GetCon(as, dirp);
                org.recast4j.recast.CompactSpan as2 = chf.spans[ai2];
                ch = Math.max(ch, as2.y);
                regs[2] = chf.spans[ai2].reg | (chf.areas[ai2] << 16);
            }
        }
        if (org.recast4j.recast.RecastCommon.GetCon(s, dirp) != RC_NOT_CONNECTED) {
            int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(dirp);
            int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(dirp);
            int ai = chf.cells[ax + ay * chf.width].index + org.recast4j.recast.RecastCommon.GetCon(s, dirp);
            org.recast4j.recast.CompactSpan as = chf.spans[ai];
            ch = Math.max(ch, as.y);
            regs[3] = chf.spans[ai].reg | (chf.areas[ai] << 16);
            if (org.recast4j.recast.RecastCommon.GetCon(as, dir) != RC_NOT_CONNECTED) {
                int ax2 = ax + org.recast4j.recast.RecastCommon.GetDirOffsetX(dir);
                int ay2 = ay + org.recast4j.recast.RecastCommon.GetDirOffsetY(dir);
                int ai2 = chf.cells[ax2 + ay2 * chf.width].index + org.recast4j.recast.RecastCommon.GetCon(as, dir);
                org.recast4j.recast.CompactSpan as2 = chf.spans[ai2];
                ch = Math.max(ch, as2.y);
                regs[2] = chf.spans[ai2].reg | (chf.areas[ai2] << 16);
            }
    ...
    private static boolean intersectSegCountour(int d0, int d1, int i, int n, int[] verts, int[] d0verts,
        ...
        d1 = 4;
        for (int k = 0; k < n; k++) {
            int k1 = org.recast4j.recast.RecastMesh.next(k, n);
            ...
            p0 = 8;
            p1 = 12;
            if (org.recast4j.recast.RecastMesh.vequal(pverts, d0, p0) || org.recast4j.recast.RecastMesh.vequal(pverts, d1, p0)
                    || org.recast4j.recast.RecastMesh.vequal(pverts, d0, p1) || org.recast4j.recast.RecastMesh.vequal(pverts, d1, p1))
                continue;

            if (org.recast4j.recast.RecastMesh.intersect(pverts, d0, d1, p0, p1))
                return true;
        }
        return false;
    }
    ...
    private static boolean inCone(int i, int n, int[] verts, int pj, int[] vertpj) {
        int pi = i * 4;
        int pi1 = org.recast4j.recast.RecastMesh.next(i, n) * 4;
        int pin1 = org.recast4j.recast.RecastMesh.prev(i, n) * 4;
        int[] pverts = new int[4 * 4];
        for (int g = 0; g < 4; g++) {
            pverts[g] = verts[pi + g];
        ...
        if (org.recast4j.recast.RecastMesh.leftOn(pverts, pin1, pi, pi1))
            return org.recast4j.recast.RecastMesh.left(pverts, pi, pj, pin1) && org.recast4j.recast.RecastMesh.left(pverts, pj, pi, pi1);
        // Assume (i-1,i,i+1) not collinear.
        // else P[i] is reflex.
        return !(org.recast4j.recast.RecastMesh.leftOn(pverts, pi, pj, pi1) && org.recast4j.recast.RecastMesh.leftOn(pverts, pj, pi, pin1));
    }
    ...
    private static void mergeContours(org.recast4j.recast.Contour ca, org.recast4j.recast.Contour cb, int ia, int ib) {
    ...
    private static int[] findLeftMostVertex(org.recast4j.recast.Contour contour) {
    ...
    private static void mergeRegionHoles(org.recast4j.recast.Context ctx, ContourRegion region) {
        ...
        org.recast4j.recast.Contour outline = region.outline;

        // Merge holes into the outline one by one.
        for (int i = 0; i < region.nholes; i++) {
            org.recast4j.recast.Contour hole = region.holes[i].contour;
    ...
    public static org.recast4j.recast.ContourSet buildContours(Context ctx, CompactHeightfield chf, float maxError, int maxEdgeLen,
                                                               int buildFlags) {

        int w = chf.width;
        int h = chf.height;
        int borderSize = chf.borderSize;
        org.recast4j.recast.ContourSet cset = new ContourSet();
            ...
            for (int x = 0; x < w; ++x) {
                ...
                for (int i = c.index, ni = c.index + c.count; i < ni; ++i) {
                    ...
                    if (simplified.size() / 4 >= 3) {
                        ...
                        org.recast4j.recast.Contour cont = new org.recast4j.recast.Contour();
                        cset.conts.add(cont);
        ...
        if (cset.conts.size() > 0) {
            ...
            for (int i = 0; i < cset.conts.size(); ++i) {
                org.recast4j.recast.Contour cont = cset.conts.get(i);
