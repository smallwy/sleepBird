...
public class RecastMeshDetail {
    ...
    private static boolean circumCircle(float[] verts, int p1, int p2, int p3, float[] c, AtomicReference<Float> r) {
        ...
        org.recast4j.recast.RecastVectors.sub(v2, verts, p2, p1);
        org.recast4j.recast.RecastVectors.sub(v3, verts, p3, p1);
        ...
        if (Math.abs(cp) > EPS) {
            ...
            r.set(vdist2(c, v1));
            org.recast4j.recast.RecastVectors.add(c, c, verts, p1);
            return true;
        }
        org.recast4j.recast.RecastVectors.copy(c, verts, p1);
        r.set(0f);
        return false;
    }
    ...
    private static float distPtTri(float[] p, float[] verts, int a, int b, int c) {
        ...
        float[] v1 = new float[3];
        float[] v2 = new float[3];
        org.recast4j.recast.RecastVectors.sub(v0, verts, c, a);
        org.recast4j.recast.RecastVectors.sub(v1, verts, b, a);
        org.recast4j.recast.RecastVectors.sub(v2, p, verts, a);
    ...
    private static int getHeight(float fx, float fy, float fz, float cs, float ics, float ch, int radius,
        ...
        ix = org.recast4j.recast.RecastCommon.clamp(ix - hp.xmin, 0, hp.width - 1);
    ...
    private static void addEdge(org.recast4j.recast.Context ctx, List<Integer> edges, int maxEdges, int s, int t, int l, int r) {
    ...
    static int completeFacet(org.recast4j.recast.Context ctx, float[] pts, int npts, List<Integer> edges, int maxEdges, int nfaces, int e) {
        float EPS = 1e-5f;
    ...
    private static void delaunayHull(org.recast4j.recast.Context ctx, int npts, float[] pts, int nhull, int[] hull, List<Integer> tris) {
        int nfaces = 0;
    ...
    private static void triangulateHull(int nverts, float[] verts, int nhull, int[] hull, int nin, List<Integer> tris) {
        ...
        for (int i = 0; i < nhull; i++) {
            ...
            // segments on edges
            int pi = org.recast4j.recast.RecastMesh.prev(i, nhull);
            int ni = org.recast4j.recast.RecastMesh.next(i, nhull);
            int pv = hull[pi] * 3;
            int cv = hull[i] * 3;
            int nv = hull[ni] * 3;
        ...
        while (org.recast4j.recast.RecastMesh.next(left, nhull) != right) {
            // Check to see if se should advance left or right.
            int nleft = org.recast4j.recast.RecastMesh.next(left, nhull);
    ...
    static int buildPolyDetail(org.recast4j.recast.Context ctx, float[] in, int nin, float sampleDist, float sampleMaxError,
        ...
        for (int i = 0; i < nin; ++i) {
            org.recast4j.recast.RecastVectors.copy(verts, i * 3, in, i * 3);
        }
        tris.clear();
            ...
            for (int i = 0, j = nin - 1; i < nin; j = i++) {
                ...
                if (swapped) {
                    for (int k = nidx - 2; k > 0; --k) {
                        org.recast4j.recast.RecastVectors.copy(verts, nverts * 3, edge, idx[k] * 3);
                        hull[nhull++] = nverts;
                        nverts++;
                    }
                } else {
        ...
        if (sampleDist > 0) {
            ...
            org.recast4j.recast.RecastVectors.copy(bmin, in, 0);
            org.recast4j.recast.RecastVectors.copy(bmax, in, 0);
            for (int i = 1; i < nin; ++i) {
                org.recast4j.recast.RecastVectors.min(bmin, in, i * 3);
                org.recast4j.recast.RecastVectors.max(bmax, in, i * 3);
            }
            ...
            for (int iter = 0; iter < nsamples; ++iter) {
                ...
                org.recast4j.recast.RecastVectors.copy(verts, nverts * 3, bestpt, 0);
                nverts++;
    ...
    static void seedArrayWithPolyCenter(org.recast4j.recast.Context ctx, CompactHeightfield chf, int[] meshpoly, int poly, int npoly,
        ...
        for (int j = 0; j < npoly && dmin > 0; ++j) {
            for (int k = 0; k < 9 && dmin > 0; ++k) {
                ...
                org.recast4j.recast.CompactCell c = chf.cells[(ax + bs) + (az + bs) * chf.width];
    ...
    static void getHeightData(org.recast4j.recast.Context ctx, CompactHeightfield chf, int[] meshpolys, int poly, int npoly, int[] verts,
    ...
    public static org.recast4j.recast.PolyMeshDetail buildPolyMeshDetail(org.recast4j.recast.Context ctx, PolyMesh mesh, CompactHeightfield chf,
        ...
        org.recast4j.recast.PolyMeshDetail dmesh = new org.recast4j.recast.PolyMeshDetail();
        int nvp = mesh.nvp;
        float cs = mesh.cs;
        float ch = mesh.ch;
    ...
    org.recast4j.recast.PolyMeshDetail mergePolyMeshDetails(Context ctx, org.recast4j.recast.PolyMeshDetail[] meshes, int nmeshes) {
        org.recast4j.recast.PolyMeshDetail mesh = new org.recast4j.recast.PolyMeshDetail();
