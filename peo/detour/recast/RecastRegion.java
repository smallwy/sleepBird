...
public class RecastRegion {
    ...
    public static int calculateDistanceField(CompactHeightfield chf, int[] src) {
        ...
        for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; ++x) {
                org.recast4j.recast.CompactCell c = chf.cells[x + y * w];
                for (int i = c.index, ni = c.index + c.count; i < ni; ++i) {
                    org.recast4j.recast.CompactSpan s = chf.spans[i];
                    int area = chf.areas[i];

                    int nc = 0;
                    for (int dir = 0; dir < 4; ++dir) {
                        if (org.recast4j.recast.RecastCommon.GetCon(s, dir) != RecastConstants.RC_NOT_CONNECTED) {
                            int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(dir);
                            int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(dir);
                            int ai = chf.cells[ax + ay * w].index + org.recast4j.recast.RecastCommon.GetCon(s, dir);
        ...
        for (int y = 0; y < h; ++y) {
            for (int x = 0; x < w; ++x) {
                ...
                for (int i = c.index, ni = c.index + c.count; i < ni; ++i) {
                    ...
                    if (org.recast4j.recast.RecastCommon.GetCon(s, 0) != RecastConstants.RC_NOT_CONNECTED) {
                        // (-1,0)
                        int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(0);
                        int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(0);
                        int ai = chf.cells[ax + ay * w].index + org.recast4j.recast.RecastCommon.GetCon(s, 0);
                        org.recast4j.recast.CompactSpan as = chf.spans[ai];
                        if (src[ai] + 2 < src[i]) {
                            src[i] = src[ai] + 2;
                        }

                        // (-1,-1)
                        if (org.recast4j.recast.RecastCommon.GetCon(as, 3) != RecastConstants.RC_NOT_CONNECTED) {
                            int aax = ax + org.recast4j.recast.RecastCommon.GetDirOffsetX(3);
                            int aay = ay + org.recast4j.recast.RecastCommon.GetDirOffsetY(3);
                            int aai = chf.cells[aax + aay * w].index + org.recast4j.recast.RecastCommon.GetCon(as, 3);
                            if (src[aai] + 3 < src[i]) {
                                src[i] = src[aai] + 3;
                            }
                        }
                    }
                    if (org.recast4j.recast.RecastCommon.GetCon(s, 3) != RecastConstants.RC_NOT_CONNECTED) {
                        // (0,-1)
                        int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(3);
                        int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(3);
                        int ai = chf.cells[ax + ay * w].index + org.recast4j.recast.RecastCommon.GetCon(s, 3);
                        org.recast4j.recast.CompactSpan as = chf.spans[ai];
                        if (src[ai] + 2 < src[i]) {
                            src[i] = src[ai] + 2;
                        }
                        ...
                        // (1,-1)
                        if (org.recast4j.recast.RecastCommon.GetCon(as, 2) != RecastConstants.RC_NOT_CONNECTED) {
                            int aax = ax + org.recast4j.recast.RecastCommon.GetDirOffsetX(2);
                            int aay = ay + org.recast4j.recast.RecastCommon.GetDirOffsetY(2);
                            int aai = chf.cells[aax + aay * w].index + org.recast4j.recast.RecastCommon.GetCon(as, 2);
                            if (src[aai] + 3 < src[i]) {
                                src[i] = src[aai] + 3;
                            }
                        }
        ...
        for (int y = h - 1; y >= 0; --y) {
            for (int x = w - 1; x >= 0; --x) {
                ...
                for (int i = c.index, ni = c.index + c.count; i < ni; ++i) {
                    ...
                    if (org.recast4j.recast.RecastCommon.GetCon(s, 2) != RecastConstants.RC_NOT_CONNECTED) {
                        // (1,0)
                        int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(2);
                        int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(2);
                        int ai = chf.cells[ax + ay * w].index + org.recast4j.recast.RecastCommon.GetCon(s, 2);
                        org.recast4j.recast.CompactSpan as = chf.spans[ai];
                        if (src[ai] + 2 < src[i]) {
                            src[i] = src[ai] + 2;
                        }
                        ...
                        // (1,1)
                        if (org.recast4j.recast.RecastCommon.GetCon(as, 1) != RecastConstants.RC_NOT_CONNECTED) {
                            int aax = ax + org.recast4j.recast.RecastCommon.GetDirOffsetX(1);
                            int aay = ay + org.recast4j.recast.RecastCommon.GetDirOffsetY(1);
                            int aai = chf.cells[aax + aay * w].index + org.recast4j.recast.RecastCommon.GetCon(as, 1);
                            if (src[aai] + 3 < src[i]) {
                                src[i] = src[aai] + 3;
                            }
                        }
                    }
                    if (org.recast4j.recast.RecastCommon.GetCon(s, 1) != RecastConstants.RC_NOT_CONNECTED) {
                        // (0,1)
                        int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(1);
                        int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(1);
                        int ai = chf.cells[ax + ay * w].index + org.recast4j.recast.RecastCommon.GetCon(s, 1);
                        org.recast4j.recast.CompactSpan as = chf.spans[ai];
                        if (src[ai] + 2 < src[i]) {
                            src[i] = src[ai] + 2;
                        }
                        ...
                        // (-1,1)
                        if (org.recast4j.recast.RecastCommon.GetCon(as, 0) != RecastConstants.RC_NOT_CONNECTED) {
                            int aax = ax + org.recast4j.recast.RecastCommon.GetDirOffsetX(0);
                            int aay = ay + org.recast4j.recast.RecastCommon.GetDirOffsetY(0);
                            int aai = chf.cells[aax + aay * w].index + org.recast4j.recast.RecastCommon.GetCon(as, 0);
                            if (src[aai] + 3 < src[i]) {
                                src[i] = src[aai] + 3;
                            }
                        }
                ...
                for (int i = c.index, ni = c.index + c.count; i < ni; ++i) {
                    ...
                    for (int dir = 0; dir < 4; ++dir) {
                        if (org.recast4j.recast.RecastCommon.GetCon(s, dir) != RecastConstants.RC_NOT_CONNECTED) {
                            ...
                            org.recast4j.recast.CompactSpan as = chf.spans[ai];
                            int dir2 = (dir + 1) & 0x3;
                            if (org.recast4j.recast.RecastCommon.GetCon(as, dir2) != RecastConstants.RC_NOT_CONNECTED) {
                                int ax2 = ax + org.recast4j.recast.RecastCommon.GetDirOffsetX(dir2);
                                int ay2 = ay + org.recast4j.recast.RecastCommon.GetDirOffsetY(dir2);
                                int ai2 = chf.cells[ax2 + ay2 * w].index + org.recast4j.recast.RecastCommon.GetCon(as, dir2);
                                d += src[ai2];
                            } else {
                                d += cd;
                            }
    ...
    private static boolean floodRegion(int x, int y, int i, int level, int r, CompactHeightfield chf, int[] srcReg,
        ...
        while (stack.size() > 0) {
            ...
            org.recast4j.recast.CompactSpan cs = chf.spans[ci];

            // Check if any of the neighbours already have a valid region set.
            int ar = 0;
            for (int dir = 0; dir < 4; ++dir) {
                // 8 connected
                if (org.recast4j.recast.RecastCommon.GetCon(cs, dir) != RecastConstants.RC_NOT_CONNECTED) {
                    int ax = cx + org.recast4j.recast.RecastCommon.GetDirOffsetX(dir);
                    int ay = cy + org.recast4j.recast.RecastCommon.GetDirOffsetY(dir);
                    int ai = chf.cells[ax + ay * w].index + org.recast4j.recast.RecastCommon.GetCon(cs, dir);
    ...
    private static boolean isSolidEdge(CompactHeightfield chf, int[] srcReg, int x, int y, int i, int dir) {
        ...
        if (org.recast4j.recast.RecastCommon.GetCon(s, dir) != RecastConstants.RC_NOT_CONNECTED) {
            int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(dir);
            int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(dir);
            int ai = chf.cells[ax + ay * chf.width].index + org.recast4j.recast.RecastCommon.GetCon(s, dir);
            r = srcReg[ai];
        }
        if (r == srcReg[i]) {
            return false;
    ...
    private static void walkContour(int x, int y, int i, int dir, CompactHeightfield chf, int[] srcReg,
        ...
        org.recast4j.recast.CompactSpan ss = chf.spans[i];
        int curReg = 0;
        if (org.recast4j.recast.RecastCommon.GetCon(ss, dir) != RecastConstants.RC_NOT_CONNECTED) {
            int ax = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(dir);
            int ay = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(dir);
            int ai = chf.cells[ax + ay * chf.width].index + org.recast4j.recast.RecastCommon.GetCon(ss, dir);
            curReg = srcReg[ai];
        }
        cont.add(curReg);
        ...
        while (++iter < 40000) {
            ...
            if (isSolidEdge(chf, srcReg, x, y, i, dir)) {
                ...
                int ni = -1;
                int nx = x + org.recast4j.recast.RecastCommon.GetDirOffsetX(dir);
                int ny = y + org.recast4j.recast.RecastCommon.GetDirOffsetY(dir);
                if (org.recast4j.recast.RecastCommon.GetCon(s, dir) != RecastConstants.RC_NOT_CONNECTED) {
                    org.recast4j.recast.CompactCell nc = chf.cells[nx + ny * chf.width];
                    ni = nc.index + org.recast4j.recast.RecastCommon.GetCon(s, dir);
                }
                if (ni == -1) {
                    // Should not happen.
                    return;
    ...
    private static int mergeAndFilterRegions(org.recast4j.recast.Context ctx, int minRegionArea, int mergeRegionSize, int maxRegionId,
    ...
    private static int mergeAndFilterLayerRegions(org.recast4j.recast.Context ctx, int minRegionArea, int maxRegionId,
                                                  CompactHeightfield chf, int[] srcReg, List<Integer> overlaps) {
        int w = chf.width;
        int h = chf.height;
    ...
    public static void buildDistanceField(org.recast4j.recast.Context ctx, CompactHeightfield chf) {
    ...
    public static void buildRegionsMonotone(org.recast4j.recast.Context ctx, CompactHeightfield chf, int borderSize, int minRegionArea,
                                            int mergeRegionArea) {
    ...
    ///
    /// @see rcCompactHeightfield, rcCompactSpan, rcBuildDistanceField, rcBuildRegionsMonotone, rcConfig
    public static void buildRegions(org.recast4j.recast.Context ctx, CompactHeightfield chf, int borderSize, int minRegionArea,
                                    int mergeRegionArea) {
        ctx.startTimer("BUILD_REGIONS");
