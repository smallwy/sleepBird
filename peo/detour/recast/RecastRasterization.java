...
public class RecastRasterization {
     ...
     *
     * @see org.recast4j.recast.Heightfield , Span.
     */
    private static void addSpan(org.recast4j.recast.Heightfield hf, int x, int y, int smin, int smax, int area, int flagMergeThr) {

        int idx = x + y * hf.width;

        org.recast4j.recast.Span s = new org.recast4j.recast.Span();
        s.smin = smin;
        s.smax = smax;
        s.area = area;
        s.next = null;
        ...
        org.recast4j.recast.Span prev = null;
        org.recast4j.recast.Span cur = hf.spans[idx];
    ...
    private static int[] dividePoly(float[] buf, int in, int nin, int out1, int out2, float x, int axis) {
        ...
        for (int i = 0, j = nin - 1; i < nin; j = i, ++i) {
            ...
            if (ina != inb) {
                ...
                org.recast4j.recast.RecastVectors.copy(buf, out2 + n * 3, buf, out1 + m * 3);
                m++;
                n++;
                // add the i'th point to the right polygon. Do NOT add points that are on the dividing line
                // since these were already added above
                if (d[i] > 0) {
                    org.recast4j.recast.RecastVectors.copy(buf, out1 + m * 3, buf, in + i * 3);
                    m++;
                } else if (d[i] < 0) {
                    org.recast4j.recast.RecastVectors.copy(buf, out2 + n * 3, buf, in + i * 3);
                    n++;
                }
            } else // same side
            {
    ...
    private static void rasterizeTri(float[] verts, int v0, int v1, int v2, int area, org.recast4j.recast.Heightfield hf, float[] bmin,
        ...
        org.recast4j.recast.RecastVectors.copy(tmin, verts, v0 * 3);
        org.recast4j.recast.RecastVectors.copy(tmax, verts, v0 * 3);
        org.recast4j.recast.RecastVectors.min(tmin, verts, v1 * 3);
        org.recast4j.recast.RecastVectors.min(tmin, verts, v2 * 3);
        org.recast4j.recast.RecastVectors.max(tmax, verts, v1 * 3);
        org.recast4j.recast.RecastVectors.max(tmax, verts, v2 * 3);
        ...
        y0 = org.recast4j.recast.RecastCommon.clamp(y0, 0, h - 1);
        y1 = org.recast4j.recast.RecastCommon.clamp(y1, 0, h - 1);
        ...
        org.recast4j.recast.RecastVectors.copy(buf, 0, verts, v0 * 3);
        org.recast4j.recast.RecastVectors.copy(buf, 3, verts, v1 * 3);
        ...
        for (int y = y0; y <= y1; ++y) {
            ...
            x0 = org.recast4j.recast.RecastCommon.clamp(x0, 0, w - 1);
            x1 = org.recast4j.recast.RecastCommon.clamp(x1, 0, w - 1);
            ...
            for (int x = x0; x <= x1; ++x) {
                ...
                int ismin = org.recast4j.recast.RecastCommon.clamp((int) Math.floor(smin * ich), 0, RecastConstants.RC_SPAN_MAX_HEIGHT);
                int ismax = RecastCommon.clamp((int) Math.ceil(smax * ich), ismin + 1, RecastConstants.RC_SPAN_MAX_HEIGHT);
     ...
     *
     * @see org.recast4j.recast.Heightfield
     */
    public static void rasterizeTriangle(org.recast4j.recast.Context ctx, float[] verts, int v0, int v1, int v2, int area,
                                         org.recast4j.recast.Heightfield solid, int flagMergeThr) {
     ...
     * @see org.recast4j.recast.Heightfield
     */
    public static void rasterizeTriangles(org.recast4j.recast.Context ctx, float[] verts, int[] tris, int[] areas, int nt,
                                          org.recast4j.recast.Heightfield solid, int flagMergeThr) {
